...
public:
    ...
    int dfs(int u, string& colors) {
        // If u is marked as cycle, there is some cycle.
        if (cycle[u]) return INT_MAX;
        if (vis[u]) return freq[u][colors[u]-'a'];

        vis[u]=cycle[u]=1;// mark visited & cycle
        ...
        // backtracking, u may not cause a cycle.
        cycle[u]=0;
        return ++freq[u][colors[u]-'a']; // increase freq 
    }
